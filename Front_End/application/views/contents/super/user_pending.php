...
<div class="card-box mb-30">
    <div class="pd-20">
        <h4 class="text-blue h4">ALL PENDING USERS</h4>
    </div>
    <div class="pb-20">
        <table class="table hover data-table-export nowrap" export="Files">
            ...
            <tbody>
                <?php foreach($this->users_tbl->showAllPendingAdmin() as $col): ?>
                    <tr>
                        <td></td>
                        <td>
                            <button class="btn btn-success" style="border-radius: 50%;height:28px; width:28px;text-align:center;padding:0px;font-size:0px;" onclick="acceptUser('<?= ENCRYPT($col['emp_id']) ?>', '<?= $col['fullname'] ?>', '<?= ENCRYPT($col['username']) ?>')" data="" title="Accept file"><span class="icon-copy ti-check-box text-white" style="font-size:16px;"></span></button>
